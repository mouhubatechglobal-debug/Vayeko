...
/** Menu mobile plein écran (hamburger) — monté via Portal directement dans le <body>. */
export function MobileMenu() {
  ...
  const menuContent = open && mounted ? (
    <div
      role="dialog"
      aria-modal="true"
      className="fixed inset-0 z-[99999] flex flex-col p-6 text-white overflow-y-auto"
    ...
    >
      ...
      {/* Liens de navigation */}
      <nav aria-label="Navigation mobile" className="mt-8 flex flex-col gap-3">
        {MAIN_NAV.map((item) => (
          <Link
            key={item.href}
            href={item.href}
  ...
  return (
    <div className="md:hidden">
      ...
      {mounted && typeof document !== 'undefined' && menuContent ? createPortal(menuContent, document.body) : null}
    </div>
  );
}
